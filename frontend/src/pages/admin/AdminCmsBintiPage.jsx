...
export function AdminCmsBintiPage() {
  ...
  async function refresh() {
    const d = await adminFetch("/api/admin/cms/binti");
    setPosts(d.posts ?? []);
  }
  ...
  async function handleSave(payload, id) {
    ...
    if (id) {
      await adminFetch(`/api/admin/cms/binti/${id}`, { method: "PUT", body });
    } else {
      await adminFetch("/api/admin/cms/binti", { method: "POST", body });
    }
    await refresh();
  ...
  async function handleDelete(id) {
    await adminFetch(`/api/admin/cms/binti/${id}`, { method: "DELETE" });
    await refresh();
    setModal({ type: null, item: null });
  }
    ...
    <div>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-3 text-2xl font-semibold">Binti</h1>
...
function BintiModal({ title, initial, onClose, onSave }) {
